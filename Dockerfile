...
# Python 3.12 via deadsnakes PPA (Ubuntu 22.04 ships 3.10; SGLang requires 3.12)
RUN apt-get update && \
...
RUN python3.12 -m pip install --no-cache-dir ".[all]"
...
RUN chmod +x /entrypoint.sh && sed -i 's/\r$//' /entrypoint.sh && \
    python3.12 -c "import sglang; print('SGLang', sglang.__version__, 'OK')"
